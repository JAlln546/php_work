...
{# shows form so user can enter their name.  Initial scenario
	...
	<form action="' . THIS_PAGE . '" method="post" onsubmit="return checkForm(this);">
		...
		foreach($config->items as $item)
          ...
          echo '
				<p>
					<input type="submit" value="Order Items"><em>&nbsp; (<font color="dodgerblue"><b>*</b> required field</font>)</em>
				</p>
    ...
    {//loop the form elements
        ...
        if(substr($name,0,5)=='item_')
        {
            ...
            $id = (int)$name_array[1];
					...
					if($value > 0){
						$thisItem = getItem($id);
						$itemSubtotal = $value * $thisItem->Price;
						$subTotal += $itemSubtotal;



						echo "<p>You ordered $value $thisItem->Name(s) which is $$itemSubtotal.</p>";
					}
			...
			*/
            //echo "<p>You ordered $value of item number $id</p>";
				...
				$value += $value;
			//	$itemSubtotal = $subTotal += $subTotal;
				//$tax = $itemTotals * .1;
				//$total = $itemTotals + $tax;

				//echo "<p>Item Total: $$itemTotals</p>";
				//echo "<p>Calculated Tax: $$tax</p>";
				//echo "<p>Total Bill: $$total</p>";
    }
$countExtras = $count * .25;
$subTotal = round($subTotal + $countExtras, 2);
$tax = round($subTotal * .1, 2);
$total = round($subTotal + $tax, 2);

echo "<br>";
echo "<h3>Bill Details</h3>";
echo "<p><strong>Add On Extras Total:</strong>&nbsp; $$countExtras</p>";
echo "<p><strong>Items Total:</strong>&nbsp; $$subTotal</p>";
...
function getItem($id) {
	...
	/*
		...
		if/when we find the item with the id that matches
	...
	foreach($config->items as $item)
				{
					...
					if($id == (int)$item->ID){ //this is our item returning
						return $item;
					}
						//echo '<p>' . $item->Name . ' <input type="text" name="item_' . $item->ID . '" /></p>';
